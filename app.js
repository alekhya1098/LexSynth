...
window.addEventListener('DOMContentLoaded', () => {
  ...
  renderCompareAnglePicker();
  checkSharedMemo();
  checkBackend();
  // Init compare mode hidden
  document.getElementById('compare-results') && document.getElementById('compare-results').classList.add('hidden');
});
...
function setMode(mode) {
  ...
  document.getElementById('compare-results').classList.add('hidden');
  if (mode === 'compare') renderCompareAnglePicker();
}

function renderCompareAnglePicker() {
  document.getElementById('compare-angle-checkboxes').innerHTML = ANGLES.map(a => `
    <label class="angle-check">
      <input type="checkbox" value="${a.id}" ${['statutory','practical','recent'].includes(a.id) ? 'checked' : ''} />
      ${a.label}
    </label>`).join('');
}

function getCompareAngles() {
  return ANGLES.filter(a => document.querySelector(`#compare-angle-checkboxes input[value="${a.id}"]`)?.checked);
}

// ── Compare flow ──────────────────────────────────────────────────────────────
async function startCompare() {
  const cfg = getConfig();
  const q1 = document.getElementById('compare-q1').value.trim();
  const q2 = document.getElementById('compare-q2').value.trim();
  const j1 = document.getElementById('compare-j1').options[document.getElementById('compare-j1').selectedIndex].text;
  const j2 = document.getElementById('compare-j2').options[document.getElementById('compare-j2').selectedIndex].text;
  const angles = getCompareAngles();

  if (!q1 || !q2) { showError('Please enter both questions.'); return; }
  if (q1.length < 10 || q2.length < 10) { showError('Each question must be at least 10 characters.'); return; }
  if (!cfg.apiKey) { showError('Please enter your API key in settings.'); document.getElementById('settings-details').open = true; return; }
  if (!angles.length) { showError('Select at least one research angle.'); return; }

  const btn = document.getElementById('compare-btn');
  const spinner = document.getElementById('compare-spinner');
  document.getElementById('compare-btn-text').textContent = 'Researching...';
  ...
  try {
    ...
    const [resA, resB] = await Promise.all([
      runCompareQuery(q1, j1, angles, cfg),
      runCompareQuery(q2, j2, angles, cfg),
    ]);

    renderCompareResults(q1, j1, resA, q2, j2, resB);
    document.getElementById('compare-results').classList.remove('hidden');
    document.getElementById('compare-results').scrollIntoView({ behavior: 'smooth' });
  } catch (e) {
    showError(`Comparison failed: ${e.message}`);
  } finally {
    document.getElementById('compare-btn-text').textContent = '⚖️ Compare Both';
    spinner.classList.add('hidden');
    btn.disabled = false;
  }
}

async function runCompareQuery(question, jurisdiction, selectedAngles, cfg) {
  // Get sources
  let sources = [];
  try {
    if (cfg.searchKey) {
      const raw = await searchWeb(`${question} ${jurisdiction} law`, cfg.searchKey);
      sources = raw.slice(0, 5).map((r, i) => ({ num: i+1, title: r.title, url: r.link, snippet: r.snippet || '' }));
    } else {
      const sim = await simulateSearch(question, jurisdiction, cfg);
      sources = sim.slice(0, 5).map((r, i) => ({ num: i+1, title: r.title, url: r.url, snippet: r.snippet }));
    }
  } catch { sources = []; }

  const sourceText = sources.map(s => `[${s.num}] ${s.title}\n${s.url}\n${s.snippet}`).join('\n\n');

  // Run angles (use only first 2 for speed in compare mode)
  const limitedAngles = selectedAngles.slice(0, 2);
  const angleResults = [];
  for (const a of limitedAngles) {
    try {
      const text = await callLLM([{ role: 'user', content: buildDirectAnglePrompt(a.id, question, jurisdiction, sourceText) }], cfg);
      angleResults.push({ id: a.id, label: a.label, text });
    } catch (e) {
      angleResults.push({ id: a.id, label: a.label, text: `Unavailable: ${e.message}` });
    }
    await new Promise(r => setTimeout(r, 4000));
  }

  // Accuracy
  let accuracy = fallbackAccuracy();
  try {
  ...
  let memo = '';
  try {
    memo = await callLLM([{ role: 'user', content:
      `Senior legal analyst. Write a SHORT legal research summary (max 300 words) for this question.
Question: ${question} | Jurisdiction: ${jurisdiction}
Key findings:\n${angleResults.map(a => `${a.label}: ${a.text.slice(0,300)}`).join('\n')}
Include: Short Answer, Key Law, Main Risk. End with: Not legal advice.` }], cfg);
  } catch (e) { memo = `Summary unavailable: ${e.message}`; }

  return { sources, angleResults, accuracy, memo };
}

function renderCompareResults(q1, j1, resA, q2, j2, resB) {
  document.getElementById('compare-col-label-a').textContent = `A: ${q1.slice(0, 60)}${q1.length > 60 ? '…' : ''}`;
  document.getElementById('compare-col-label-b').textContent = `B: ${q2.slice(0, 60)}${q2.length > 60 ? '…' : ''}`;

  // Accuracy badges
  document.getElementById('compare-accuracy-a').innerHTML = renderMiniAccuracy(resA.accuracy, j1);
  document.getElementById('compare-accuracy-b').innerHTML = renderMiniAccuracy(resB.accuracy, j2);

  // Memos
  document.getElementById('compare-memo-a').innerHTML = mdToHtml(resA.memo);
  document.getElementById('compare-memo-b').innerHTML = mdToHtml(resB.memo);

  // Diff bar — highlight confidence difference
  const confMap = { high: 3, medium: 2, low: 1 };
  const cA = confMap[resA.accuracy?.confidence] || 2;
  const cB = confMap[resB.accuracy?.confidence] || 2;
  const diffMsg = cA === cB
    ? `Both questions have <strong>${resA.accuracy?.confidence || 'medium'}</strong> confidence answers.`
    : `Question ${cA > cB ? 'A' : 'B'} has a <strong>higher confidence</strong> answer (${resA.accuracy?.confidence} vs ${resB.accuracy?.confidence}).`;
  document.getElementById('compare-diff-bar').innerHTML = `<span>📊 ${diffMsg}</span>`;
}

function renderMiniAccuracy(acc, jurisdiction) {
  if (!acc) return '';
  const tier = acc.confidence || 'medium';
  const score = Math.round(((acc.source_agreement?.score || 50) + (acc.legal_certainty?.score || 50)) / 2);
  return `<div class="mini-accuracy">
    <span class="mini-badge ${tier}">${tier.toUpperCase()}</span>
    <span class="mini-jurisdiction">${escHtml(jurisdiction)}</span>
    <div class="acc-bar-wrap" style="margin-top:6px"><div class="acc-bar ${tier}" style="width:${score}%"></div></div>
    <p class="mini-verdict">${escHtml(acc.verdict || '')}</p>
  </div>`;
}
